...
function _renderListContentOnly() {
    ...
    if (currentMajorTab === 'reply') {
        ...
        else if (currentSubTab === 'catchNight') itemsToRender = customCatchNight;
...
function _renderAtmosphereList(list, items) {
    ...
    const indexMaps = {
        ...
        catchNight: new Map(customCatchNight.map((r,i) => [r, i])),
    };
    const frag = document.createDocumentFragment();
...
function _runDedup() {
    ...
    const ccnDedup = deduplicateContentArray(customCatchNight);
    customCatchNight = ccnDedup.result; totalRemoved += ccnDedup.removedCount;
...
function deleteItem(index) {
    ...
    else if (currentSubTab === 'catchNight') customCatchNight.splice(index, 1);
    if (item && ctx.groups) {
...
function editItem(index, oldText) {
    ...
    else if (currentSubTab === 'catchNight') customCatchNight[index] = newText.trim();
    throttledSaveData();
    renderReplyLibrary();
}
...
function _showExportUI() {
    ...
    const modules = [
        ...
        { id: '_re_cn',       icon: ICONS.dot,       label: '抓熬夜通知',    count: customCatchNight.length,                  key: 'customCatchNight' },
...
function _doExport(selectedModules) {
    ...
    selectedModules.forEach(m => {
        ...
        else if (m.key === 'customCatchNight'){ libraryData.customCatchNight  = customCatchNight;                libraryData.modules.push('catchNight'); }
...
function _showImportUI(data) {
    const knownFields = ['customReplies','customPokes','customStatuses','customMottos','customIntros','customEmojis','customPeriodCare','customCatchNight',
    ...
    const modules = [
        ...
        { id: '_ri_cn',       icon: ICONS.dot,       label: '抓熬夜通知',    data: data.customCatchNight,     key: 'customCatchNight' },
        ...
        try {
            ...
            if (overwrite) {
                selected.forEach(m => {
                    ...
                    else if (m.key === 'customCatchNight'){ customCatchNight           = data.customCatchNight;   totalAdded += data.customCatchNight.length; }
                ...
                selected.forEach(m => {
                    ...
                    } else if (m.key === 'customCatchNight') {
                        const before = customCatchNight.length;
                        customCatchNight = deduplicateContentArray([...customCatchNight, ...data.customCatchNight]).result;
                        totalAdded += customCatchNight.length - before;
    ...
    panel.querySelector('#ba-confirm').onclick = () => {
        ...
        lines.forEach(val => {
            ...
            const isDup = currentSubTab === 'custom'
                ...
                : currentSubTab === 'catchNight'
                ? customCatchNight.some(r => normalizeStringStrict(r) === norm)
                : false;
            ...
            else if (currentSubTab === 'catchNight') customCatchNight.push(val);
            added++;
        });
    ...
    if (addBtn) {
        addBtn.addEventListener('click', () => {
            ...
            if (input?.trim()) {
                ...
                else if (currentSubTab === 'catchNight' && customCatchNight.some(r => normalizeStringStrict(r) === valNorm)) isDup = true;
                ...
                else if (currentSubTab === 'catchNight') customCatchNight.unshift(val);
                throttledSaveData(); renderReplyLibrary();
...
function getCategoryName(tabId) {
    return { custom: '回复', pokes: '拍一拍', statuses: '状态', mottos: '格言', intros: '开场语', period: '相关字卡', catchNight: '抓熬夜通知' }[tabId] || '内容';
}
